...
static int doInflateMemoryWithHint(unsigned char *in, unsigned int inLength, unsigned char **out, unsigned int *outLength, unsigned int outlengthHint) {
    /* ret value */
    int err = Z_OK;
...
int inflateMemoryWithHint(unsigned char *in, unsigned int inLength, unsigned char **out, unsigned int outLengthHint) {
    ...
    int err = doInflateMemoryWithHint(in, inLength, out, &outLength, outLengthHint );
...
int inflateGZipFile(const char *path, unsigned char **out) {
    ...
    NSCAssert(out, @"inflateGZipFile: invalid 'out' parameter");
    NSCAssert(&*out, @"inflateGZipFile: invalid 'out' parameter");
    
    gzFile inFile = gzopen(path, "rb");
    if (inFile == NULL) {
        NSLog(@"ZipUtils: error open gzip file: %s", path);
        return -1;
    }
    ...
    *out = malloc(bufferSize);
    ...
    for (;;) {
        ...
        if (len < 0) {
            NSLog(@"ZipUtils: error in gzread");
            free( *out );
            *out = NULL;
            return -1;
        }
    ...
    if (gzclose(inFile) != Z_OK) {
        NSLog(@"ZipUtils: gzclose failed");
    }
    
    return offset;
}

int inflateCCZFile(const char *path, unsigned char **out) {
    NSCAssert(out, @"inflateCCZFile: invalid 'out' parameter");
    NSCAssert(&*out, @"inflateCCZFile: invalid 'out' parameter");
    
    // load file into memory
    unsigned char *compressed = NULL;
    NSInteger fileLen  = loadFileIntoMemory( path, &compressed );
    if (fileLen < 0) {
        NSLog(@"Error loading CCZ compressed file");
        return -1;
    }
